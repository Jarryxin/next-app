...
const MessageBubble = memo(function MessageBubble({
  ...
  function getProviderLabel(modelId: string): string {
    for (const g of groups) {
      if (g.models.some((m) => m.id === modelId)) return g.providerLabel;
    }
    return "Agnes AI";
  }
          ...
          <>
            {msg.model && (
              <div className="mb-1 text-[10px] font-medium text-zinc-400 uppercase tracking-wide">
                {msg.model.startsWith("dashscope-")
                  ? `百炼 DashScope · ${getModelLabel(msg.model)}`
...
export default function ChatContent({ modelGroups: groups }: { modelGroups: ModelGroup[] }) {
